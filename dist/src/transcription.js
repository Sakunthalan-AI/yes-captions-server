...
function groupWordsByTiming(words) {
    ...
    if (avgGap < 0.05)
        timingThreshold = FAST_SPEECH_THRESHOLD;
    else if (avgGap < 0.1)
        timingThreshold = FAST_SPEECH_THRESHOLD + 0.02;
    else if (avgGap > 0.25)
        timingThreshold = SLOW_SPEECH_THRESHOLD;
    for (let i = 0; i < words.length; i++) {
        const word = words[i];
        const nextWord = words[i + 1];
        let roundedStart = roundTo(word.start ?? 0, 2);
        let roundedEnd = roundTo(word.end ?? roundedStart, 2);
        if (!Number.isFinite(roundedStart))
            roundedStart = 0;
        if (!Number.isFinite(roundedEnd))
        ...
        if (hasPunctuation)
            forceEndGroup = true;
        else if (hasComma && currentGroup.length >= 2)
            shouldEndGroup = true;
        if (currentGroup.length > 0) {
            const groupStart = currentGroup[0].start;
            const groupEnd = roundedWord.end;
            if (groupEnd - groupStart >= MAX_GROUP_DURATION)
                shouldEndGroup = true;
        }
        if (currentGroup.length >= MAX_WORDS_PER_GROUP)
            shouldEndGroup = true;
        if (nextWord) {
            const gap = nextWord.start - word.end;
            if (gap > timingThreshold * 2)
                forceEndGroup = true;
            else if (gap > timingThreshold && currentGroup.length >= MIN_WORDS_PER_GROUP)
                shouldEndGroup = true;
        }
        else {
            forceEndGroup = true;
        ...
        if ((forceEndGroup || shouldEndGroup) && currentGroup.length > 0) {
            groups.push({
                ...
                end: Math.max(currentGroup[currentGroup.length - 1].end, currentGroup[0].start + MIN_SEGMENT_DURATION),
...
const normalizeSegments = (segments, totalDuration) => {
...
export async function transcribeVideo(videoPath, tempDir) {
    ...
    try {
        ...
        const formDataBuffer = await new Promise((resolve, reject) => {
            const chunks = [];
            groqFormData.on("data", (chunk) => chunks.push(Buffer.from(chunk)));
        ...
        const groqResponse = await fetch("https://api.groq.com/openai/v1/audio/transcriptions", {
            method: "POST",
            headers: headers,
            body: formDataBuffer, // Cast to any to satisfy type checker for Buffer vs BodyInit
        });
        if (!groqResponse.ok) {
