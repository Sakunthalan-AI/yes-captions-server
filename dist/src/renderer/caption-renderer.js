 ...
 */
async function generateHTML(payload, videoPath) {
 ...
 */
export async function captureFrames(browser, payload, videoPath, outputDir, fps = 30, quality = 85, concurrency = 4 // Restore concurrency to 4 for speed
) {
    ...
    const processChunk = async (chunkIndex, startFrame, endFrame, existingPage) => {
        const page = existingPage || await browser.newPage();
        if (!existingPage) {
            await setupPage(page, payload, videoPath, outputDir);
        }
        const chunkFramePaths = [];
 ...
 */
async function setupPage(page, payload, videoPath, outputDir) {
 ...
 */
export async function closeBrowserInstance(browser) {
